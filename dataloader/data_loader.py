...
def get_train_valid_test(df_path, valid_size=0.1, random_state=216):
    ...
    X_train, X_valid, y_train, y_valid = train_test_split(
        X_train,
        y_train,
        test_size=(valid_size / (1 - valid_size)),
        stratify=y_train,
        random_state=random_state,
    )
...
class DataLoader(Sequence):
    ...
    def __getitem__(self, index):
        image_name_batch = self.image_filenames[
        ...
        image_batch = np.zeros(
            (image_name_batch.shape[0], self.image_size, self.image_size, 3),
            dtype="float32",
        )

        for image_index in range(image_name_batch.shape[0]):
            image = cv2.cvtColor(
                cv2.imread(image_name_batch[image_index]), cv2.COLOR_BGR2RGB
            )
    ...
    def on_epoch_end(self):
        if self.shuffle:
            self.image_filenames, self.labels = sklearn.utils.shuffle(
